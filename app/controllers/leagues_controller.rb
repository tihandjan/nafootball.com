class LeaguesController < ApplicationController
  ...
  def set_onlain_fixtures
      @fixtures_en = Match.where(["DATE(date) = ? and league = ?", Time.current+1.hour, 'apl']).order('date')
        ...
        @fixtures_en_will = Match.where(["DATE(date) = ? and league = ?", Time.current+1.days+1.hour, 'apl']).order('date')
        @fixtures_ua_will = Match.where(["DATE(date) = ? and league = ?", Time.current+1.days+1.hour, 'ukraine']).order('date')
        @fixtures_ru_will = Match.where(["DATE(date) = ? and league = ?", Time.current+1.days+1.hour, 'russian']).order('date')
        @fixtures_it_will = Match.where(["DATE(date) = ? and league = ?", Time.current+1.days+1.hour, 'seria-a']).order('date')
        @fixtures_sp_will = Match.where(["DATE(date) = ? and league = ?", Time.current+1.days+1.hour, 'laliga']).order('date')
        @fixtures_ge_will = Match.where(["DATE(date) = ? and league = ?", Time.current+1.days+1.hour, 'bundesliga']).order('date')
        @fixtures_cl_will = Match.where(["DATE(date) = ? and league = ?", Time.current+1.days+1.hour, 'chempions-league']).order('date')
  end
